...
//var countFilms=filims.length
//console.log(countFilms);

// movies whose runtime below 140

//var movieRuntime=filims.filter(f=>f.runtime<140).map(f=>f.title)
//console.log(movieRuntime);

// movie with highest run time

/*var maxRuntime=filims.reduce((f1,f2)=>Number(f1.runtime)>Number(f2.runtime) ?f1 :f2)
...
/*var minRuntime=filims.reduce((f1,f2)=>Number(f1.runtime)<Number(f2.runtime) ? f1 : f2)
...
// includes used bcoz genres is an again array so belong check using includes

var movieFilter=filims.filter(f=>f.genres.includes("Action") || f.genres.includes("Drama")).map(f=>f.title)
